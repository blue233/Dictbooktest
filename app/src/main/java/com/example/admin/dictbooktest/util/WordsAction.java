...
import com.example.admin.dictbooktest.db.WordsSQLiteOpenHelper;
...
public class WordsAction {
     ...
     * 数据库工具，用于增、删、该、查
     */
    private WordsSQLiteOpenHelper helper1;
    ...
    private WordsAction(Context context) {
        WordsSQLiteOpenHelper helper1 = new WordsSQLiteOpenHelper(context, DB_WORDS, null, 1);
    ...
    public boolean saveWords(Words words) {
        ...
        if (words.getSent().length() > 0) {
            ...
            db.insert(TABLE_WORDS1, null, values);
            values.clear();
            return true;
        }
        return false;
     ...
     * @param words 单词类的实例
     */
    public boolean saveWords_loc(Words words) {
        //判断是否是有效对象，即有数据
        if (words.getSent().length() > 0&&wordsAction.getWordsFromSQLite_loc(words.getKey()).getKey()=="") {
            ContentValues values = new ContentValues();
            values.put("isChinese", "" + words.getIsChinese());
            values.put("key", words.getKey());
            values.put("fy", words.getFy());
            ...
            values.put("posAcceptation", words.getPosAcceptation());
            values.put("sent", words.getSent());
            db.insert(TABLE_WORDS2, null, values);
            values.clear();
            Log.v("测试","new词库中存入了");
            return true;
        }else{
    ...
    public Words getWordsFromSQLite(String key) {
        ...
        Cursor cursor = db.query(TABLE_WORDS1, null, "key=?", new String[]{key}, null, null, null);
        //数据库中有
     ...
     * @return words 若返回words的key为空，则说明数据库中没有该词
     */
    public Words getWordsFromSQLite_loc(String key) {
        Words words = new Words();
        Cursor cursor = db.query(TABLE_WORDS2, null, "key=?", new String[]{key}, null, null, null);
            //数据库中有
            if (cursor.getCount() > 0) {
            Log.d("测试", "数据库中有");
            if (cursor.moveToFirst()) {
 ...
 * @param temp 要删除的单词
 */
public void deleteFromSQLite_loc(String temp){
    //数据库中有
    if (wordsAction.getWordsFromSQLite_loc(temp).getKey()!="") {
        Log.d("测试", "数据库中有");
        String[] s={temp};
        db.delete(TABLE_WORDS2,"key==?",s);

    } else {
        Log.d("测试", "new数据库中没有，无法删除");
    }
}
     ...
     * 从生词本中获取单词列表
     */
    public List<String> getWordslistFromSQLite_loc() {
